// $Id: Bs2Jpsifzero_Signal_v5a.h,v 1.1 2009/12/06  Pete Clarke Exp $
/** @class Bs2Jpsifzero_Signal_v5a
 *
...
#ifndef Bs2Jpsifzero_Signal_v5a_H
#define Bs2Jpsifzero_Signal_v5a_H
...
//		double EvaluateComponent( DataPoint* input, ComponentRef* );
	...
	private:
		//Bs2Jpsifzero_Signal_v5a& operator=( const Bs2Jpsifzero_Signal_v5& );
		void MakePrototypes();
...
//		bool _usePlotComponents ;
		...
		/*	const double gl = gamma() + ( dgam *0.5 ) ;
			if( gl < 0. ) {
				PDF_THREAD_LOCK
				cerr << " In Bs2Jpsifzero_Signal_v5a : gamma_l() < 0 so setting it to 0.0000001 " << endl ;
				PDF_THREAD_UNLOCK
				return 0.0000001 ;
			}
			else
		...
		/*	const double gh = gamma() - ( dgam *0.5 ) ;
			if( gh < 0. ) {
				PDF_THREAD_LOCK
				cerr << " In Bs2Jpsifzero_Signal_v5a : gamma_h() < 0 so setting it to 0.0000001 " << endl ;
				PDF_THREAD_UNLOCK
				return 0.0000001 ;
			}
			else
		...
		inline double mistag() const {
			...
			if( (fabs(q()) < 0.5) || (fabs(q()) > 1.) ) { 
				...
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5a::mistag() : _mistag < 0 so set to 0 " << endl ;
				PDF_THREAD_UNLOCK
				returnValue = 0 ;
			}
			else if( _mistag > 0.5 ) {
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5a::mistag() : _mistag > 0.5 so set to 0.5 "  << endl ;
				PDF_THREAD_UNLOCK
				returnValue = 0.5 ;
			}
			else {
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5a::mistag() : WARNING ******If you got here you dont know what you are doing  "  << endl ;
				PDF_THREAD_UNLOCK
				exit(1);
			}
			return returnValue ;
		...
		inline double mistagB() const {
			...
			if( (fabs(q()) < 0.5) || (fabs(q()) > 1.) ) {
				...
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5a::mistagB() : _mistag < 0 so deltaMistag set to 0 also " << endl ;
				PDF_THREAD_UNLOCK
				returnValue = 0 ;
			}
			else if( _mistag > 0.5 ) {
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5a::mistagB() : _mistag > 0.5 so so deltaMistag set to 0.5 also "  << endl ;
				PDF_THREAD_UNLOCK
				returnValue = 0.5 ;
			}
			else {
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5a::mistagB() : WARNING ******If you got here you dont know what you are doing  "  << endl ;
				PDF_THREAD_UNLOCK
				exit(1);
			}
			return returnValue ;
		...
		inline double mistagBbar() const {
			...
			if( fabs(q()) < 0.5 ) {
				...
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5a::mistagBbar() : _mistag < 0 so deltaMistag set to 0 also " << endl ;
				PDF_THREAD_UNLOCK
				returnValue = 0 ;
			}
			else if( _mistag > 0.5 ) {
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5a::mistagBbar() : _mistag > 0.5 so so deltaMistag set to 0.5 also "  << endl ;
				PDF_THREAD_UNLOCK
				returnValue = 0.5 ;
			}
			else {
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5a::mistagBbar() : WARNING ******If you got here you dont know what you are doing  "  << endl ;
				PDF_THREAD_UNLOCK
				exit(1);
			}
			return returnValue ;
		...
		inline double timeFactorEven()  const
		{
			...
			if( DebugFlag_v5a )
			{
				...
				{
				PDF_THREAD_LOCK
				cout << " Bs2Jpsifzero_Signal_v5a::timeFactorEven() : result < 0 " << endl ;
