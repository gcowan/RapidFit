// $Id: Bs2Jpsifzero_Signal_v5.h,v 1.1 2009/12/06  Pete Clarke Exp $
/** @class Bs2Jpsifzero_Signal_v5
 *
...
#ifndef Bs2Jpsifzero_Signal_v5_H
#define Bs2Jpsifzero_Signal_v5_H
...
//============================================================================================
class Bs2Jpsifzero_Signal_v5 : public BasePDF
{
	public:
		Bs2Jpsifzero_Signal_v5( PDFConfigurator* );
		Bs2Jpsifzero_Signal_v5( const Bs2Jpsifzero_Signal_v5& );
		~Bs2Jpsifzero_Signal_v5();
...
//		double EvaluateComponent( DataPoint* input, ComponentRef* );
	...
	private:
		//Bs2Jpsifzero_Signal_v5& operator=( const Bs2Jpsifzero_Signal_v5& );
		void MakePrototypes();
...
//		bool _usePlotComponents ;
		...
		/*	const double gl = gamma() + ( dgam *0.5 ) ;
			if( gl < 0. ) {
				PDF_THREAD_LOCK
				cerr << " In Bs2Jpsifzero_Signal_v5 : gamma_l() < 0 so setting it to 0.0000001 " << endl ;
				PDF_THREAD_UNLOCK
				return 0.0000001 ;
			}
			else
		...
		/*	const double gh = gamma() - ( dgam *0.5 ) ;
			if( gh < 0. ) {
				PDF_THREAD_LOCK
				cerr << " In Bs2Jpsifzero_Signal_v5 : gamma_h() < 0 so setting it to 0.0000001 " << endl ;
				PDF_THREAD_UNLOCK
				return 0.0000001 ;
			}
			else
		...
		inline double mistagB() const {
			...
			if( fabs(q()) < 0.5 ) {
				...
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5::mistagB() : _mistag < 0 so deltaMistag set to 0 also " << endl ;
				PDF_THREAD_UNLOCK
				returnValue = 0 ;
			}
			else if( _mistag > 0.5 ) {
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5::mistagB() : _mistag > 0.5 so so deltaMistag set to 0.5 also "  << endl ;
				PDF_THREAD_UNLOCK
				returnValue = 0.5 ;
			}
			else {
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5::mistagB() : WARNING ******If you got here you dont know what you are doing  "  << endl ;
				PDF_THREAD_UNLOCK
				exit(1);
			}
			return returnValue ;
		...
		inline double mistagBbar() const {
			...
			if( fabs(q()) < 0.5 ) {
				...
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5::mistagBbar() : _mistag < 0 so deltaMistag set to 0 also " << endl ;
				PDF_THREAD_UNLOCK
				returnValue = 0 ;
			}
			else if( _mistag > 0.5 ) {
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5::mistagBbar() : _mistag > 0.5 so so deltaMistag set to 0.5 also "  << endl ;
				PDF_THREAD_UNLOCK
				returnValue = 0.5 ;
			}
			else {
				PDF_THREAD_LOCK
				cout << "Bs2Jpsifzero_Signal_v5::mistagBbar() : WARNING ******If you got here you dont know what you are doing  "  << endl ;
				PDF_THREAD_UNLOCK
				exit(1);
			}
			return returnValue ;
