// $Id: Bs2JpsiPhi_SignalAlt_BaseClass_v3.h,v 1.1 2009/12/06  Pete Clarke Exp $
/** @class Bs2JpsiPhi_SignalAlt_BaseClass_v3.h
 *
...
#ifndef Bs2JpsiPhi_SignalAlt_BaseClass_v3_H
#define Bs2JpsiPhi_SignalAlt_BaseClass_v3_H
...
class Bs2JpsiPhi_SignalAlt_BaseClass_v3
{
	public:
		Bs2JpsiPhi_SignalAlt_BaseClass_v3( PDFConfigurator* );
		Bs2JpsiPhi_SignalAlt_BaseClass_v3( const Bs2JpsiPhi_SignalAlt_BaseClass_v3& input );
		virtual ~Bs2JpsiPhi_SignalAlt_BaseClass_v3();

	private:
		Bs2JpsiPhi_SignalAlt_BaseClass_v3& operator=( const Bs2JpsiPhi_SignalAlt_BaseClass_v3& );
	...
	protected:
		...
		inline double gamma_l() const { 
			...
			if( gl < 0. ) {
				cerr << " In Bs2JpsiPhi_SignalAlt_BaseClass_v3 : gamma_l() < 0 so setting it to 0.0000001 " << endl ;
				return 0.0000001 ;
			}
			else
				return gl ; 
		...
		inline double gamma_h() const { 
			...
			if( gh < 0. ) {
				cerr << " In Bs2JpsiPhi_SignalAlt_BaseClass_v3 : gamma_h() < 0 so setting it to 0.0000001 " << endl ;
				return 0.0000001 ;
			}
			else
				return gh ;   
		...
		inline double mistag() const { 
			...
			if( fabs((q()-0.0)) < DOUBLE_TOLERANCE ) {
				...
				cout << "Bs2JpsiPhi_SignalAlt_BaseClass_v3::mistag() : _mistag < 0 so set to 0 " << endl ;
				returnValue = 0 ;
			}
			else if( _mistag > 0.5 ) {
				cout << "Bs2JpsiPhi_SignalAlt_BaseClass_v3::mistag() : _mistag > 0.5 so set to 0.5 "  << endl ;
				returnValue = 0.5 ;
			}
			else {
				cout << "Bs2JpsiPhi_SignalAlt_BaseClass_v3::mistag() : WARNING ******If you got here you dont know what you are doing  "  << endl ;
				exit(1);
			}
			return returnValue ;			
		}
		...
		inline double timeFactorEven(  )  const
		{
			...
			if( DEBUGFLAG && (result < 0) ) {
				cout << " Bs2JpsiPhi_SignalAlt_BaseClass_v3::timeFactorEven() : result < 0 " << endl ;
