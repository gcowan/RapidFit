// $Id: Bs2Jpsifzero_Signal_v5a.cpp,v 1.1 2009/12/06 Pete Clarke Exp $
/** @class Bs2Jpsifzero_Signal_v5a Bs2Jpsifzero_Signal_v5a.cpp
 *
...
#include "Bs2Jpsifzero_Signal_v5a.h"
...
PDF_CREATOR( Bs2Jpsifzero_Signal_v5a );


Bs2Jpsifzero_Signal_v5a::Bs2Jpsifzero_Signal_v5a( const Bs2Jpsifzero_Signal_v5a& input ) : BasePDF( (BasePDF&) input )
...
Bs2Jpsifzero_Signal_v5a::Bs2Jpsifzero_Signal_v5a(PDFConfigurator* configurator) : BasePDF(),
	// Physics parameters
	...
	//, _fitDirectlyForApara(false)
{
	...
	std::cout << "Constructing PDF: Bs2Jpsifzero_Signal_v5a " << endl;
...
/*	string angAccFile = configurator->getConfigurationValue( "AngularAcceptanceFile" ) ;
	...
	if( angAccFile == "" ) cout << "Bs2Jpsifzero_Signal_v5a:: Using flat angular acceptance " << endl ;
	else cout << "Bs2Jpsifzero_Signal_v5a:: Constructing angAcc using file: " << angAccFile << endl ;
	...
	if( _angAccIgnoreNumerator ) cout << "Bs2Jpsifzero_Signal_v5a:: Ignoring angular acceptance numerator " << endl ;
*/
	//...........................................
	...
	if( useTimeAcceptance() )
	{
		if( configurator->hasConfigurationValue( "TimeAcceptanceType", "Upper" ) )
		{
			...
			cout << "Bs2Jpsifzero_Signal_v5a:: Constructing timeAcc: Upper time acceptance beta=0.00826 [0 < t < 14] " << endl ;
		}
		else if( configurator->getConfigurationValue( "TimeAcceptanceFile" ) != "" )
		{
			...
			cout << "Bs2Jpsifzero_Signal_v5a:: Constructing timeAcc: using file: " << configurator->getConfigurationValue( "TimeAcceptanceFile" ) << endl ;
		}
	}
	...
	if( timeAcc == NULL )
	{
		...
		cout << "Bs2Jpsifzero_Signal_v5a:: Constructing timeAcc: DEFAULT FLAT [0 < t < 20]  " << endl ;
	}
...
void Bs2Jpsifzero_Signal_v5a::SetupAngularTerms()
{
...
//........................................................
//Destructor
Bs2Jpsifzero_Signal_v5a::~Bs2Jpsifzero_Signal_v5a()
{
...
void Bs2Jpsifzero_Signal_v5a::MakePrototypes()
{
...
bool Bs2Jpsifzero_Signal_v5a::SetPhysicsParameters( ParameterSet* NewParameterSet )
{
...
/*
	...
	*/
	...
	Aperp_sq = 1.0;
	/*
	if( (Aperp_sq < 0.) || (Aperp_sq > 1.)  ) { cout << "Warning in Bs2Jpsifzero_Signal_v5a::SetPhysicsParameters: Aperp_sq <0 or >1 but left as is" <<  endl ;	}
	...
	if( Apara_sq < 0. )
	{
		cout << "Warning in Bs2Jpsifzero_Signal_v5a::SetPhysicsParameters: derived parameter Apara_sq <0  and so set to zero" <<  endl ;
		Apara_sq = 0. ;
	}
	...
	if( (fs < 0.) || (fs >= 1.) )
	{
		cout << "Warning in Bs2Jpsifzero_Signal_v5a::SetPhysicsParameters: As_sq <0 or >=1 but left as is" <<  endl ;
	}
	As_sq = fs / (1. - fs );
...
void Bs2Jpsifzero_Signal_v5a::prepareTimeFac()
{
...
double Bs2Jpsifzero_Signal_v5a::EvaluateForNumericIntegral(DataPoint * measurement)
{
...
double Bs2Jpsifzero_Signal_v5a::Evaluate(DataPoint * measurement)
{
...
//	this->CacheAmplitudesAndAngles() ;
	...
	if( !performingComponentProjection )
	{
		if( DebugFlag_v5a )
		{
			...
			if( c1 || c2 || c3 )
			{
				this->DebugPrint( " Bs2JpsiPhi_Signal_v5a::Evaluate() returns <=0 or nan :" , returnValue ) ;
...
double Bs2Jpsifzero_Signal_v5a::EvaluateTimeOnly(DataPoint * measurement)
{
	_datapoint = measurement;
/*
	A0A0_value = measurement->GetPseudoObservable( A0A0_Obs );
...
*/
	...
	if( DebugFlag_v5a )
	{
		...
		if( (c1 || c2 || c3)  )
		{
			this->DebugPrint( " Bs2JpsiPhi_Signal_v5a::EvaluateTimeOnly() returns <=0 or nan :" , returnValue ) ;
			if( std::isnan(returnValue) ) throw 10 ;
			if( returnValue <= 0. ) throw 10 ;
		}
	}
...
double Bs2Jpsifzero_Signal_v5a::Normalisation(DataPoint * measurement, PhaseSpaceBoundary * boundary)
{
	_datapoint = measurement;
	...
	if( !performingComponentProjection )
	{
		if( DebugFlag_v5a )
		{
			...
			if( c1 || c2 )
			{
				this->DebugPrint( " Bs2JpsiPhi_Signal_v5a::Normalisation() returns <=0 or nan :" , returnValue ) ;
				if( std::isnan(returnValue) ) throw 10 ;
				if( returnValue <= 0. ) throw 10 ;
			}
		}
...
void Bs2Jpsifzero_Signal_v5a::preCalculateTimeFactors()
{
	if( useEventResolution() )
	{
...
//.......................................................
// Pre calculate the time integrals : this is becaue these functions are called many times for each event due to the 10 angular terms
void Bs2Jpsifzero_Signal_v5a::preCalculateTimeIntegrals()
{
...
/*
vector<string> Bs2Jpsifzero_Signal_v5a::PDFComponents()
{
...
double Bs2Jpsifzero_Signal_v5a::EvaluateComponent( DataPoint* input, ComponentRef* Component )
{
...
double Bs2Jpsifzero_Signal_v5a::diffXsec()
{
...
*/
	...
	if( DebugFlag_v5a )
	{
		if( xsec < 0)
		{
			this->DebugPrintXsec( " Bs2JpsiPhi_Signal_v5a_v1::diffXsec( ) : return value < 0 = ", xsec );
		}
	}
...
double Bs2Jpsifzero_Signal_v5a::diffXsecTimeOnly()
{
	preCalculateTimeFactors() ;
...
//		ASint()*A0() * timeFactorReASA0(  ) * angAccI10 ;
	...
	if( DebugFlag_v5a )
	{
		if( xsec < 0 )
		{
			this->DebugPrintXsec( " Bs2JpsiPhi_Signal_v5a_v1::diffXsecTimeOnly( ) : return value < 0 = ", xsec );
		}
	}
...
double Bs2Jpsifzero_Signal_v5a::diffXsecNorm1()
{
...
//		ASint()*A0() * timeFactorReASA0Int(  ) * angAccI10 ;
	...
	if( DebugFlag_v5a )
	{
		if( norm < 0 )
		{
			this->DebugPrintNorm( " Bs2JpsiPhi_Signal_v5a_v1::diffXsecNorm1( ) : return value < 0 = ", norm );
		}
	}
	return norm ;
}
...
double Bs2Jpsifzero_Signal_v5a::diffXsecCompositeNorm1( int resolutionIndex )
{
...
/*
void Bs2Jpsifzero_Signal_v5a::CacheAmplitudesAndAngles()
{
...
//.......................................................
// New speed up method to Cache time integrals
void Bs2Jpsifzero_Signal_v5a::deCacheTimeIntegrals( unsigned int ires, unsigned int islice )
{
...
void Bs2Jpsifzero_Signal_v5a::prepareCDS()
{
...
void Bs2Jpsifzero_Signal_v5a::DebugPrint( string message, double value )  const
{
	PDF_THREAD_LOCK
	...
	if( !performingComponentProjection )
	{
		...
		cout << "*************DEBUG OUTPUT FROM Bs2Jpsifzero_Signal_v5a::DebugPrint ***************************" << endl ;
...
void Bs2Jpsifzero_Signal_v5a::DebugPrintNorm( string message, double value )  const
{
	PDF_THREAD_LOCK
