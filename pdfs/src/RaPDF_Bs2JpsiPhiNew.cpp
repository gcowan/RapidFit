// $Id: RaPDF_Bs2JpsiPhiNew.cpp,v 1.1 2009/11/10 10:35:49 gcowan Exp $
/** @class RaPDF_Bs2JpsiPhiNew RaPDF_Bs2JpsiPhiNew.cpp
 *
...
//Constructor
RaPDF_Bs2JpsiPhiNew::RaPDF_Bs2JpsiPhiNew() : 
	// Physics parameters
	  gammaName     ( "gamma" )
...
void RaPDF_Bs2JpsiPhiNew::MakePrototypes()
{
...
bool RaPDF_Bs2JpsiPhiNew::SetPhysicsParameters( ParameterSet * NewParameterSet )
{
	normalisationCacheValid = false;
	evaluationCacheValid = false;
...
vector<string> RaPDF_Bs2JpsiPhiNew::GetDoNotIntegrateList()
{
...
double RaPDF_Bs2JpsiPhiNew::Normalisation(DataPoint * measurement, PhaseSpaceBoundary * boundary)
{
	// Now need to know the tag and the mistag
    int q = (int)measurement->GetObservable( tagName )->GetValue();
    double omega = measurement->GetObservable( mistagName )->GetValue();	
...
void RaPDF_Bs2JpsiPhiNew::getAngularFunctions( double & f1
					, double & f2
					, double & f3
					, double & f4
					, double & f5
...
void RaPDF_Bs2JpsiPhiNew::getTimeDependentAmplitudes(  double & AzeroAzero
						, double & AparaApara
						, double & AperpAperp
						, double & ImAparaAperp
						, double & ReAzeroApara
...
void RaPDF_Bs2JpsiPhiNew::getTimeAmplitudeIntegrals( double & AzeroAzeroInt
...
inline double RaPDF_Bs2JpsiPhiNew::getAzeroAzeroInt(double tmin, double tmax, 
...
inline double RaPDF_Bs2JpsiPhiNew::getAparaAparaInt(double tmin, double tmax,
					double k0, double tauL, double tauH, double tauBar,double Dms, double phis, int Btype)
{
...
inline double RaPDF_Bs2JpsiPhiNew::getAperpAperpInt(double tmin, double tmax,
							   double k0, double tauL, double tauH, double tauBar,double Dms, double phis, int Btype)
{
...
inline double RaPDF_Bs2JpsiPhiNew::A4def(double tmin, double tmax,
...
inline double RaPDF_Bs2JpsiPhiNew::A5def(double tmin, double tmax,
					double k0, double tauL, double tauH, double tauBar, 
					double Dms, double phis, double tphase1, double tphase2, int Btype)
{
...
void RaPDF_Bs2JpsiPhiNew::getPhysicsParameters( double & gamma
					, double & deltaGamma
					, double & deltaM
					, double & Phi_s
					, double & Azero_sq
