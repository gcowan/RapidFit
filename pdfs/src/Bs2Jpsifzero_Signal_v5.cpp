// $Id: Bs2Jpsifzero_Signal_v5.cpp,v 1.1 2009/12/06 Pete Clarke Exp $
/** @class Bs2Jpsifzero_Signal_v5 Bs2Jpsifzero_Signal_v5.cpp
 *
...
#include "Bs2Jpsifzero_Signal_v5.h"
...
Bs2Jpsifzero_Signal_v5::Bs2Jpsifzero_Signal_v5(PDFConfigurator* configurator) : BasePDF(),
	// Physics parameters
	...
	//, _fitDirectlyForApara(false)
{
	...
	std::cout << "Constructing PDF: Bs2Jpsifzero_Signal_v5 " << endl;
...
/*	string angAccFile = configurator->getConfigurationValue( "AngularAcceptanceFile" ) ;
	...
	if( angAccFile == "" ) cout << "Bs2Jpsifzero_Signal_v5:: Using flat angular acceptance " << endl ;
	else cout << "Bs2Jpsifzero_Signal_v5:: Constructing angAcc using file: " << angAccFile << endl ;
	...
	if( _angAccIgnoreNumerator ) cout << "Bs2Jpsifzero_Signal_v5:: Ignoring angular acceptance numerator " << endl ;
*/
	//...........................................
	...
	if( useTimeAcceptance() ) {
		if( configurator->hasConfigurationValue( "TimeAcceptanceType", "Upper" ) ) {
			...
			cout << "Bs2Jpsifzero_Signal_v5:: Constructing timeAcc: Upper time acceptance beta=0.0112 [0 < t < 14] " << endl ;
		}
			...
			cout << "Bs2Jpsifzero_Signal_v5:: Constructing timeAcc: using file: " << configurator->getConfigurationValue( "TimeAcceptanceFile" ) << endl ;
		}
	}
	else {
...
void Bs2Jpsifzero_Signal_v5::SetupAngularTerms()
{
...
//........................................................
//Destructor
Bs2Jpsifzero_Signal_v5::~Bs2Jpsifzero_Signal_v5()
{
...
void Bs2Jpsifzero_Signal_v5::MakePrototypes()
{
...
vector<string> Bs2Jpsifzero_Signal_v5::GetDoNotIntegrateList()
{
	vector<string> list;
...
bool Bs2Jpsifzero_Signal_v5::SetPhysicsParameters( ParameterSet* NewParameterSet )
{
...
/*
	...
	if( (Azero_sq < 0.) || (Azero_sq > 1.)  ) { cout << "Warning in Bs2Jpsifzero_Signal_v5::SetPhysicsParameters: Azero_sq <0 or >1 but left as is" <<  endl ;	}
	*/
	//Aperp_sq = allParameters.GetPhysicsParameter( Aperp_sqName )->GetValue();
	Aperp_sq = 1.0;
	/*
	if( (Aperp_sq < 0.) || (Aperp_sq > 1.)  ) { cout << "Warning in Bs2Jpsifzero_Signal_v5::SetPhysicsParameters: Aperp_sq <0 or >1 but left as is" <<  endl ;	}
	...
	if( Apara_sq < 0. ) {
		cout << "Warning in Bs2Jpsifzero_Signal_v5::SetPhysicsParameters: derived parameter Apara_sq <0  and so set to zero" <<  endl ;
		Apara_sq = 0. ;
	}

	double fs = allParameters.GetPhysicsParameter( As_sqName )->GetValue();
	if( (fs < 0.) || (fs >= 1.) ) { cout << "Warning in Bs2Jpsifzero_Signal_v5::SetPhysicsParameters: As_sq <0 or >=1 but left as is" <<  endl ;	}
	As_sq = fs / (1. - fs ) ;
...
double Bs2Jpsifzero_Signal_v5::EvaluateForNumericIntegral(DataPoint * measurement)
{
...
double Bs2Jpsifzero_Signal_v5::Evaluate(DataPoint * measurement)
{
...
//	this->CacheAmplitudesAndAngles() ;
	...
	if( DEBUGFLAG && (c1 || c2 || c3)  ) {
		this->DebugPrint( " Bs2Jpsifzero_Signal_v5::Evaluate() returns <=0 or nan :" , returnValue ) ;
...
double Bs2Jpsifzero_Signal_v5::EvaluateTimeOnly(DataPoint * measurement)
{
	_datapoint = measurement;
/*
	A0A0_value = measurement->GetPseudoObservable( A0A0_Obs );
...
*/
	...
	bool c3 = (resolutionScale<=0.) && (t>0.) && (returnValue <= 0.)  ;
	if( DEBUGFLAG && (c1 || c2 || c3)  ) {
		this->DebugPrint( " Bs2Jpsifzero_Signal_v5::EvaluateTimeOnly() returns <=0 or nan :" , returnValue ) ;
		if( std::isnan(returnValue) ) throw 10 ;
		if( returnValue <= 0. ) throw 10 ;
	}
...
double Bs2Jpsifzero_Signal_v5::Normalisation(DataPoint * measurement, PhaseSpaceBoundary * boundary)
{
	_datapoint = measurement;
	...
	if( DEBUGFLAG && (c1 || c2 ) ) {
		//this->DebugPrint( " Bs2Jpsifzero_Signal_v5::Normalisation() returns <=0 or nan :" , returnValue ) ;
		//this->DebugPrint( " Bs2Jpsifzero_Signal_v5::Normalisation() returns <=0 or nan :" , val1 ) ;
		this->DebugPrint( " Bs2Jpsifzero_Signal_v5::diffxsecnorm1 :" , AT()) ;
		if( std::isnan(returnValue) ) throw 10 ;
		if( returnValue <= 0. ) throw 10 ;
	}
...
void Bs2Jpsifzero_Signal_v5::preCalculateTimeFactors()
{
	if( useEventResolution() )
	{
...
//.......................................................
// Pre calculate the time integrals : this is becaue these functions are called many times for each event due to the 10 angular terms
void Bs2Jpsifzero_Signal_v5::preCalculateTimeIntegrals()
{
...
/*
vector<string> Bs2Jpsifzero_Signal_v5::PDFComponents()
{
...
double Bs2Jpsifzero_Signal_v5::EvaluateComponent( DataPoint* input, ComponentRef* Component )
{
...
double Bs2Jpsifzero_Signal_v5::diffXsec()
{
...
//				CachedA10 * timeFactorReASA0(  );
	...
	if( DEBUGFLAG && (xsec < 0) ) this->DebugPrintXsec( " Bs2Jpsifzero_Signal_v5_v1::diffXsec( ) : return value < 0 = ", xsec ) ;
...
double Bs2Jpsifzero_Signal_v5::diffXsecTimeOnly()
{
	preCalculateTimeFactors() ;
...
//		ASint()*A0() * timeFactorReASA0(  ) * angAccI10 ;
	...
	if( DEBUGFLAG && (xsec < 0) ) this->DebugPrintXsec( " Bs2Jpsifzero_Signal_v5_v1::diffXsecTimeOnly( ) : return value < 0 = ", xsec ) ;
...
double Bs2Jpsifzero_Signal_v5::diffXsecNorm1()
{
...
double Bs2Jpsifzero_Signal_v5::diffXsecCompositeNorm1( int resolutionIndex )
{
...
/*
void Bs2Jpsifzero_Signal_v5::CacheAmplitudesAndAngles()
{
...
//.......................................................
// New speed up method to Cache time integrals
void Bs2Jpsifzero_Signal_v5::CacheTimeIntegrals() {
...
//.......................................................
// New speed up method to Cache time integrals
void Bs2Jpsifzero_Signal_v5::deCacheTimeIntegrals( unsigned int ires, unsigned int islice )
{
...
void Bs2Jpsifzero_Signal_v5::prepareCDS()
{
...
void Bs2Jpsifzero_Signal_v5::DebugPrint( string message, double value )  const
{
	PDF_THREAD_LOCK

		(void) message; (void) value;
	cout << "*************DEBUG OUTPUT FROM Bs2Jpsifzero_Signal_v5::DebugPrint ***************************" << endl ;
...
void Bs2Jpsifzero_Signal_v5::DebugPrintXsec( string message, double value )  const
{
	PDF_THREAD_LOCK

		(void) message; (void) value;
	cout << "*************DEBUG OUTPUT FROM Bs2Jpsifzero_Signal_v5::DebugPrintXsec ***************************" << endl ;
	cout << message << value << endl <<endl ;
...
void Bs2Jpsifzero_Signal_v5::DebugPrintNorm( string message, double value )  const
{
	PDF_THREAD_LOCK

		(void) message; (void) value;
	cout << "*************DEBUG OUTPUT FROM Bs2Jpsifzero_Signal_v5::DebugPrintNorm ***************************" << endl ;
	cout << message << value << endl <<endl ;
