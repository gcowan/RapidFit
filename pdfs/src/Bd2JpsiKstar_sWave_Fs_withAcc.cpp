...
Bd2JpsiKstar_sWave_Fs_withAcc::Bd2JpsiKstar_sWave_Fs_withAcc( const Bd2JpsiKstar_sWave_Fs_withAcc& input ) : BasePDF( (BasePDF&) input ),
	...
	Apara_sq(input.Apara_sq), Aperp_sq(input.Aperp_sq), Csp(input.Csp), CspAs(input.CspAs), _plotAllComponents(input._plotAllComponents), histo(NULL), xaxis(NULL),
	yaxis(NULL), zaxis(NULL), nxbins(input.nxbins), nybins(input.nybins), nzbins(input.nzbins), xmin(input.xmin), ymin(input.ymin), zmin(input.zmin),
	xmax(input.xmax), ymax(input.ymax), zmax(input.zmax), deltax(input.deltax), deltay(input.deltay), deltaz(input.deltaz), total_num_entries(input.total_num_entries), _datapoint(NULL),
...
double Bd2JpsiKstar_sWave_Fs_withAcc::angularFactor( )
{
    ...
    const int j_max(4);
    const int k_max(4);
    double c[i_max+1][j_max+1][k_max+1];
...
c[4][0][0] = -0.203926;// +- 0.019563
    ...
    for ( int i = 0; i < i_max+1; i++ )
    {
        for ( int k = 0; k < k_max+1; k++ )
        {
            for ( int j = k; j < j_max+1; j++ ) // must have l >= k
            {
