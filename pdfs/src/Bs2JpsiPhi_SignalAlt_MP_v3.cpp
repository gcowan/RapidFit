// $Id: Bs2JpsiPhi_SignalAlt_MP_v3.cpp,v 1.1 2009/12/06 Pete Clarke Exp $
/** @class Bs2JpsiPhi_SignalAlt_MP_v3 Bs2JpsiPhi_SignalAlt_MP_v3.cpp
 *
...
PDF_CREATOR( Bs2JpsiPhi_SignalAlt_MP_v3 );

//......................................
//Constructor(s)
//...........
// New with configurator
Bs2JpsiPhi_SignalAlt_MP_v3::Bs2JpsiPhi_SignalAlt_MP_v3( PDFConfigurator* config) : 
Bs2JpsiPhi_SignalAlt_BaseClass_v3(config)
{
	MakePrototypes();	
	std::cout << "Constructing PDF: Bs2JpsiPhi_SignalAlt_MP_v3 " << std::endl ;
}


//.......................................
//Make the data point and parameter set
void Bs2JpsiPhi_SignalAlt_MP_v3::MakePrototypes()
{
...
Bs2JpsiPhi_SignalAlt_MP_v3::Bs2JpsiPhi_SignalAlt_MP_v3( const Bs2JpsiPhi_SignalAlt_MP_v3& input )
:	BasePDF( input ), Bs2JpsiPhi_SignalAlt_BaseClass_v3( input )
{
}

//........................................................
//Destructor
Bs2JpsiPhi_SignalAlt_MP_v3::~Bs2JpsiPhi_SignalAlt_MP_v3()
{
}
...
bool Bs2JpsiPhi_SignalAlt_MP_v3::SetPhysicsParameters( ParameterSet * NewParameterSet )
{
	...
	if( (Azero_sq < 0.) || (Azero_sq > 1.)  ) { cout << "Warning in Bs2JpsiPhi_SignalAlt_MP_v3::SetPhysicsParameters: Azero_sq <0 or >1 but left as is" <<  endl ;	}	
	Aperp_sq = allParameters.GetPhysicsParameter( Aperp_sqName )->GetValue();
	if( (Aperp_sq < 0.) || (Aperp_sq > 1.)  ) { cout << "Warning in Bs2JpsiPhi_SignalAlt_MP_v3::SetPhysicsParameters: Aperp_sq <0 or >1 but left as is" <<  endl ;	}	
	As_sq = allParameters.GetPhysicsParameter( As_sqName )->GetValue();
	
	if( allowNegativeAsSq ) {
		if( (As_sq > 1.) ) { cout << "Warning in Bs2JpsiPhi_SignalAlt_MP_v3::SetPhysicsParameters: As_sq >1 but left as is" <<  endl ;	}
		Apara_sq = (1. - Azero_sq - Aperp_sq ) ;
	}
	else {
		if( (As_sq < 0.) || (As_sq > 1.) ) { cout << "Warning in Bs2JpsiPhi_SignalAlt_MP_v3::SetPhysicsParameters: As_sq <0 or >1 but left as is" <<  endl ;	}	
		Apara_sq = (1. - Azero_sq - Aperp_sq  - As_sq) ;
	}
	
	if( Apara_sq < 0. ) {
		cout << "Warning in Bs2JpsiPhi_SignalAlt_MP_v3::SetPhysicsParameters: derived parameter Apara_sq <0  and so set to zero" <<  endl ;
		Apara_sq = 0. ;
	}
...
vector<string> Bs2JpsiPhi_SignalAlt_MP_v3::GetDoNotIntegrateList()
{
	vector<string> list;
...
double Bs2JpsiPhi_SignalAlt_MP_v3::EvaluateForNumericIntegral(DataPoint * measurement) 
{
...
double Bs2JpsiPhi_SignalAlt_MP_v3::Evaluate(DataPoint * measurement) 
{
	...
	if( DEBUGFLAG && (c1 || c2 || c3)  ) {
		cout << endl ;
		cout << " Bs2JpsiPhi_SignalAlt_MP_v3::evaluate() returns <=0 or nan :" << returnValue << endl ;
...
double Bs2JpsiPhi_SignalAlt_MP_v3::EvaluateTimeOnly(DataPoint * measurement) 
{
	// Get observables into member variables
	t = measurement->GetObservable( timeName )->GetValue() - timeOffset ;
	//ctheta_tr = measurement->GetObservable( cosThetaName )->GetValue();
	...
	if( DEBUGFLAG && (c1 || c2 || c3)  ) {
		cout << endl ;
		cout << " Bs2JpsiPhi_SignalAlt_MP_v3::EvaluateTimeOnly() returns <=0 or nan :" << returnValue << endl ;
		cout << "   gamma " << gamma() << endl ;
		cout << "   gl    " << gamma_l() << endl ;
		cout << "   gh    " << gamma_h()  << endl;
		cout << "   AT^2    " << AT()*AT() << endl;
...
double Bs2JpsiPhi_SignalAlt_MP_v3::Normalisation(DataPoint * measurement, PhaseSpaceBoundary * boundary) 
{
	...
	if( DEBUGFLAG && (c1 || c2 ) ) {
		cout << endl ;
		cout << " Bs2JpsiPhi_SignalAlt_MP_v3::Normaisation() returns <=0 or nan :" << returnValue << endl ;
		cout << "   gamma " << gamma() << endl ;
		cout << "   gl    " << gamma_l() << endl ;
		cout << "   gh    " << gamma_h()  << endl;
		cout << "   AT^2    " << AT()*AT() << endl;
