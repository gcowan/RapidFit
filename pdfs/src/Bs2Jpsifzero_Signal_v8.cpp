// $Id: Bs2Jpsifzero_Signal_v8.cpp,v 1.1 2009/12/06 Pete Clarke Exp $
/** @class Bs2Jpsifzero_Signal_v8 Bs2Jpsifzero_Signal_v8.cpp
 *
...
#include "Bs2Jpsifzero_Signal_v8.h"
...
PDF_CREATOR( Bs2Jpsifzero_Signal_v8 );

//......................................
//Constructor(s)
//New one with configurator
Bs2Jpsifzero_Signal_v8::Bs2Jpsifzero_Signal_v8(PDFConfigurator* configurator) : BasePDF(), 
	// Physics parameters
	...
	_useBetaParameter(false) 
{
	...
	if( !isCopy ) std::cout << "Constructing PDF: Bs2Jpsifzero_Signal_v8 " << endl;
	...
	if( !_useBetaParameter )
	{
		...
		else
		{
			...
			if( !isCopy ) cout << "Bs2Jpsifzero_Signal_v8:: Adding OffsetToGammaForBetaFactor = " << _offsetToGammaForBetaFactor << endl ;
		}
	}
...
/*	string angAccFile = configurator->getConfigurationValue( "AngularAcceptanceFile" ) ;
	...
	if( !isCopy )
	{
		if( angAccFile == "" ) cout << "Bs2Jpsifzero_Signal_v8:: Using flat angular acceptance " << endl ;
		else cout << "Bs2Jpsifzero_Signal_v8:: Constructing angAcc using file: " << angAccFile << endl ;
		...
		if( _angAccIgnoreNumerator ) cout << "Bs2Jpsifzero_Signal_v8:: Ignoring angular acceptance numerator " << endl ;
	}
	else
	{
...
//........................................................
//Destructor
Bs2Jpsifzero_Signal_v8::~Bs2Jpsifzero_Signal_v8()
{
...
void Bs2Jpsifzero_Signal_v8::MakePrototypes()
{
	//............................
...
vector<string> Bs2Jpsifzero_Signal_v8::GetDoNotIntegrateList()
{
	vector<string> list;
...
bool Bs2Jpsifzero_Signal_v8::SetPhysicsParameters( ParameterSet* NewParameterSet )
{
...
/*
	...
	{
		cout << "Warning in Bs2Jpsifzero_Signal_v8::SetPhysicsParameters: Azero_sq <0 or >1 but left as is" <<  endl ;
	}
	*/
...
if(_cpEven){
	...
	if( Apara_sq < 0. )
	{
		cout << "Warning in Bs2Jpsifzero_Signal_v8::SetPhysicsParameters: derived parameter Apara_sq <0  and so set to zero" <<  endl ;
		Apara_sq = 0. ;
	}
	...
	if( (fs < 0.) || (fs >= 1.) )
	{
		cout << "Warning in Bs2Jpsifzero_Signal_v8::SetPhysicsParameters: As_sq <0 or >=1 but left as is" <<  endl ;
	}
	As_sq = fs / (1. - fs );
...
double Bs2Jpsifzero_Signal_v8::EvaluateForNumericIntegral(DataPoint * measurement)
{
...
double Bs2Jpsifzero_Signal_v8::Evaluate(DataPoint * measurement)
{
...
/*	if( !_useHelicityBasis )
	{
		...
		A0A0_value = Bs2Jpsifzero_Angular_Terms::TangleFactorA0A0( angularData );
		APAP_value = Bs2Jpsifzero_Angular_Terms::TangleFactorAPAP( angularData );
		ATAT_value = Bs2Jpsifzero_Angular_Terms::TangleFactorATAT( angularData );
		ASAS_value = Bs2Jpsifzero_Angular_Terms::TangleFactorASAS( angularData );
		ImAPAT_value = Bs2Jpsifzero_Angular_Terms::TangleFactorImAPAT( angularData );
		ReA0AP_value = Bs2Jpsifzero_Angular_Terms::TangleFactorReA0AP( angularData );
		ImA0AT_value = Bs2Jpsifzero_Angular_Terms::TangleFactorImA0AT( angularData );
		ReASAP_value = Bs2Jpsifzero_Angular_Terms::TangleFactorReASAP( angularData );
		ImASAT_value = Bs2Jpsifzero_Angular_Terms::TangleFactorImASAT( angularData );
		ReASA0_value = Bs2Jpsifzero_Angular_Terms::TangleFactorReASA0( angularData );
	}
	else
	{
		...
		A0A0_value = Bs2Jpsifzero_Angular_Terms::HangleFactorA0A0( angularData );
		APAP_value = Bs2Jpsifzero_Angular_Terms::HangleFactorAPAP( angularData );
		ATAT_value = Bs2Jpsifzero_Angular_Terms::HangleFactorATAT( angularData );
		ASAS_value = Bs2Jpsifzero_Angular_Terms::HangleFactorASAS( angularData );
		ImAPAT_value = Bs2Jpsifzero_Angular_Terms::HangleFactorImAPAT( angularData );
		ReA0AP_value = Bs2Jpsifzero_Angular_Terms::HangleFactorReA0AP( angularData );
		ImA0AT_value = Bs2Jpsifzero_Angular_Terms::HangleFactorImA0AT( angularData );
		ReASAP_value = Bs2Jpsifzero_Angular_Terms::HangleFactorReASAP( angularData );
		ImASAT_value = Bs2Jpsifzero_Angular_Terms::HangleFactorImASAT( angularData );
		ReASA0_value = Bs2Jpsifzero_Angular_Terms::HangleFactorReASA0( angularData );
	}
...
//	this->CacheAmplitudesAndAngles() ;
	...
	if( !performingComponentProjection )
	{
		if( DebugFlag_v8 )
		{
			...
			if( c1 || c3 )
			{
				this->DebugPrint( " Bs2Jpsifzero_Signal_v8::Evaluate() returns <=0 or nan :" , returnValue ) ;
...
double Bs2Jpsifzero_Signal_v8::EvaluateTimeOnly(DataPoint * measurement)
{
	_datapoint = measurement;
...
*/
	...
	if( DebugFlag_v8 )
	{
		...
		if( (c1 || c3)  )
		{
			this->DebugPrintEvaluate( " Bs2Jpsifzero_Signal_v8::EvaluateTimeOnly() returns <=0 or nan :" , returnValue ) ;
			if( std::isnan(returnValue) ) throw 10 ;
			if( returnValue <= 0. ) throw 10 ;
		}
	}
...
double Bs2Jpsifzero_Signal_v8::Normalisation(DataPoint * measurement, PhaseSpaceBoundary * boundary)
{
	_datapoint = measurement;
	...
	if( !performingComponentProjection )
	{
		if( DebugFlag_v8 )
		{
			...
			if( c1 || c2 )
			{
				this->DebugPrintNormalisation( " Bs2Jpsifzero_Signal_v8::Normalisation() returns <=0 or nan :" , returnValue ) ;
				if( std::isnan(returnValue) ) throw 10 ;
				if( returnValue <= 0. ) throw 10 ;
			}
		}
...
void Bs2Jpsifzero_Signal_v8::preCalculateTimeFactors()
{
...
//.......................................................
// Pre calculate the time integrals : this is becaue these functions are called many times for each event due to the 10 angular terms
void Bs2Jpsifzero_Signal_v8::preCalculateTimeIntegrals()
{
	intExpL_stored = resolutionModel->ExpInt( tlo, thi, gamma_l() );
	intExpH_stored = resolutionModel->ExpInt( tlo, thi, gamma_h() );
}

void Bs2Jpsifzero_Signal_v8::preCalculateSinusoidIntegrals()
{
	if( _eventIsTagged)// && RequireInterference )
	{
...
void Bs2Jpsifzero_Signal_v8::preCalculateTimeIntegralsDebug() const
{
...
/*
vector<string> Bs2Jpsifzero_Signal_v8::PDFComponents()
{
...
double Bs2Jpsifzero_Signal_v8::EvaluateComponent( DataPoint* input, ComponentRef* Component )
{
...
double Bs2Jpsifzero_Signal_v8::diffXsec()
{
...
//	}
	...
	if( DebugFlag_v8 )
	{
		if( xsec < 0)
		{
			this->DebugPrintXsec( " Bs2Jpsifzero_Signal_v8_v1::diffXsec( ) : return value < 0 = ", xsec );
		}
	}
...
double Bs2Jpsifzero_Signal_v8::diffXsecTimeOnly()
{
	preCalculateTimeFactors() ;
...
//		ASint()*A0() * timeFactorReASA0(  ) * angAccI10 ;
	...
	if( DebugFlag_v8 )
	{
		if( xsec < 0 )
		{
			this->DebugPrintXsec( " Bs2Jpsifzero_Signal_v8_v1::diffXsecTimeOnly( ) : return value < 0 = ", xsec );
		}
	}
...
double Bs2Jpsifzero_Signal_v8::diffXsecNorm1()
{
	double norm =
...
//		ASint()*A0() * timeFactorReASA0Int(  ) * angAccI10 ;
	...
	if( DebugFlag_v8 )
	{
		if( norm < 0 )
		{
			this->DebugPrintNorm( " Bs2Jpsifzero_Signal_v8_v1::diffXsecNorm1( ) : return value < 0 = ", norm );
		}
	}
	return norm ;
}

void Bs2Jpsifzero_Signal_v8::generateTimeIntegrals()
{
	this->preCalculateTimeIntegrals();
}

void Bs2Jpsifzero_Signal_v8::generateSinusoidIntegrals()
{
	this->preCalculateSinusoidIntegrals();
}

void Bs2Jpsifzero_Signal_v8::ConstructTimeIntegrals()
{
...
double Bs2Jpsifzero_Signal_v8::diffXsecCompositeNorm1(  )
{
...
/*
void Bs2Jpsifzero_Signal_v8::CacheAmplitudesAndAngles()
{
...
void Bs2Jpsifzero_Signal_v8::prepareCDS( double lambdaVal, double PhisVal )
{
...
void Bs2Jpsifzero_Signal_v8::DebugPrintEvaluate( string message, double value )
{
	PDF_THREAD_LOCK
/*
...
void Bs2Jpsifzero_Signal_v8::DebugPrintNormalisation( string message, double value )
{
	PDF_THREAD_LOCK
/*
...
// Debug printout
//===========================================================================================
void Bs2Jpsifzero_Signal_v8::DebugPrint( string message, double value )  const
{
	if( !performingComponentProjection )
	{
		(void) message; (void) value;
		cout << "*************DEBUG OUTPUT FROM Bs2Jpsifzero_Signal_v8::DebugPrint ***************************" << endl ;
...
void Bs2Jpsifzero_Signal_v8::DebugPrintXsec( string message, double value )
{
	PDF_THREAD_LOCK
		...
		{
			(void) message; (void) value;
			cout << "*************DEBUG OUTPUT FROM Bs2Jpsifzero_Signal_v8::DebugPrintXsec ***************************" << endl;
			cout << message << value << endl << endl;
...
void Bs2Jpsifzero_Signal_v8::DebugPrintNorm( string message, double value )
{
	PDF_THREAD_LOCK
		...
		{
			(void) message; (void) value;
			cout << "*************DEBUG OUTPUT FROM Bs2Jpsifzero_Signal_v8::DebugPrintNorm ***************************" << endl;
			cout << message << value << endl << endl;
/*
			cout << endl;
...
double Bs2Jpsifzero_Signal_v8::timeFactorEven()  const
{
	if( _eventIsTagged )
	{
		return
...
void Bs2Jpsifzero_Signal_v8::timeFactorEvenDebug() const
{
...
double Bs2Jpsifzero_Signal_v8::timeFactorEvenInt()  const
{
	if( _eventIsTagged )
	{
		return
...
void Bs2Jpsifzero_Signal_v8::timeFactorEvenIntDebug() const
{
...
double Bs2Jpsifzero_Signal_v8::timeFactorOdd(  )   const
{
	if( _eventIsTagged )
	{
		return
...
double Bs2Jpsifzero_Signal_v8::timeFactorOddInt(  )  const
{
	if( _eventIsTagged )
	{
		return
...
double Bs2Jpsifzero_Signal_v8::timeFactorA0A0( )
{
...
double Bs2Jpsifzero_Signal_v8::timeFactorA0A0Int( )
{
	this->prepareCDS( lambda_zeroVal, phis_zeroVal );
	if( _eventIsTagged )
	{
...
double Bs2Jpsifzero_Signal_v8::timeFactorAPAP( )
{
...
double Bs2Jpsifzero_Signal_v8::timeFactorAPAPInt( )
{
        this->prepareCDS( lambda_paraVal, phis_paraVal );
        if( _eventIsTagged )
        {
...
double Bs2Jpsifzero_Signal_v8::timeFactorATAT( )
{
...
double Bs2Jpsifzero_Signal_v8::timeFactorATATInt( )
{
        this->prepareCDS( lambda_perpVal, phis_perpVal );
        if( _eventIsTagged )
        {
...
double Bs2Jpsifzero_Signal_v8::timeFactorImAPAT( )
{
...
double Bs2Jpsifzero_Signal_v8::timeFactorImAPATInt( )
{
	this->prepareCDS( sqrt(lambda_paraVal*lambda_perpVal), 0.5*(phis_paraVal+phis_perpVal) );
	if( _eventIsTagged )
	{
...
double Bs2Jpsifzero_Signal_v8::timeFactorReA0AP( )
{
...
double Bs2Jpsifzero_Signal_v8::timeFactorReA0APInt( )
{
        this->prepareCDS( sqrt(lambda_zeroVal*lambda_perpVal), 0.5*(phis_zeroVal+phis_perpVal) );
        if( _eventIsTagged )
        {
...
double Bs2Jpsifzero_Signal_v8::timeFactorImA0AT(  )
{
...
double Bs2Jpsifzero_Signal_v8::timeFactorImA0ATInt( )
{
	this->prepareCDS( sqrt(lambda_zeroVal*lambda_paraVal), 0.5*(phis_zeroVal+phis_paraVal) );
	if( _eventIsTagged )
	{
...
double Bs2Jpsifzero_Signal_v8::timeFactorASAS( )
{
...
double Bs2Jpsifzero_Signal_v8::timeFactorASASInt( )
{
        this->prepareCDS( lambda_SVal, phis_SVal );
        if( _eventIsTagged )
        {
...
double Bs2Jpsifzero_Signal_v8::timeFactorReASAP( )
{
...
double Bs2Jpsifzero_Signal_v8::timeFactorReASAPInt( )
{
	this->prepareCDS( sqrt(lambda_SVal*lambda_perpVal), 0.5*(phis_SVal+phis_perpVal) );
	if( _eventIsTagged )
	{
...
double Bs2Jpsifzero_Signal_v8::timeFactorImASAT( )
{
...
double Bs2Jpsifzero_Signal_v8::timeFactorImASATInt( )
{
        this->prepareCDS( sqrt(lambda_SVal*lambda_paraVal), 0.5*(phis_SVal+phis_paraVal) );
        if( _eventIsTagged )
        {
...
double Bs2Jpsifzero_Signal_v8::timeFactorReASA0( )
{
...
double Bs2Jpsifzero_Signal_v8::timeFactorReASA0Int( )
{
	this->prepareCDS( sqrt(lambda_SVal*lambda_zeroVal), 0.5*(phis_SVal+phis_zeroVal) );
	if( _eventIsTagged )
	{
