// $Id: Bs2JpsiPhi_Signal_v5_Robs.cpp,v 1.1 2009/12/06 Pete Clarke Exp $
/** @class Bs2JpsiPhi_Signal_v5_Robs Bs2JpsiPhi_Signal_v5_Robs.cpp
 *
...
#include "Bs2JpsiPhi_Signal_v5_Robs.h"
#include <iostream>
#include "math.h"
#include "TMath.h"
#include "TF1.h"
...
PDF_CREATOR( Bs2JpsiPhi_Signal_v5_Robs );

//......................................
//Constructor(s)
//New one with configurator
Bs2JpsiPhi_Signal_v5_Robs::Bs2JpsiPhi_Signal_v5_Robs(PDFConfigurator* configurator) :
	// Physics parameters
	...
	resolution(), eventResolution(),timeIntegralCacheValid(), storeExpL(), storeExpH(), storeExpSin(), storeExpCos(), normalisationCacheUntagged()
{
	...
	if( _angAccIgnoreNumerator ) cout << "Bs2JpsiPhi_Signal_v5_Robs:: Ignoring angular acceptance numerator " << endl;
	...
	if( useTimeAcceptance() ) {
		if( configurator->hasConfigurationValue( "TimeAcceptanceType", "Upper" ) ) {
			...
			cout << "Bs2JpsiPhi_Signal_v5_Robs:: Constructing timeAcc: Upper time acceptance beta=0.0112 [0 < t < 14] " << endl;
		}
			...
			cout << "Bs2JpsiPhi_Signal_v5_Robs:: Constructing timeAcc: using file: " << configurator->getConfigurationValue( "TimeAcceptanceFile" ) << endl;
		}
	}
	else {
		timeAcc = new SlicedAcceptance( 0., 14. );
		cout << "Bs2JpsiPhi_Signal_v5_Robs:: Constructing timeAcc: DEFAULT FLAT [0 < t < 14]  " << endl;
	}
...
//........................................................
//Destructor
Bs2JpsiPhi_Signal_v5_Robs::~Bs2JpsiPhi_Signal_v5_Robs() {}


//......................................
//Make the data point and parameter set
void Bs2JpsiPhi_Signal_v5_Robs::MakePrototypes()
{
...
void Bs2JpsiPhi_Signal_v5_Robs::SetupAngularTerms()
{
...
vector<string> Bs2JpsiPhi_Signal_v5_Robs::GetDoNotIntegrateList()
{
	vector<string> list;
...
bool Bs2JpsiPhi_Signal_v5_Robs::SetPhysicsParameters( ParameterSet * NewParameterSet )
{
	...
	if( (Azero_sq < 0.) || (Azero_sq > 1.)  ) { cout << "Warning in Bs2JpsiPhi_Signal_v5_Robs::SetPhysicsParameters: Azero_sq <0 or >1 but left as is" <<  endl;	}
	Aperp_sq = allParameters.GetPhysicsParameter( Aperp_sqName )->GetValue();
	if( (Aperp_sq < 0.) || (Aperp_sq > 1.)  ) { cout << "Warning in Bs2JpsiPhi_Signal_v5_Robs::SetPhysicsParameters: Aperp_sq <0 or >1 but left as is" <<  endl;	}

	Apara_sq = (1. - Azero_sq - Aperp_sq );
	if( Apara_sq < 0. ) {
		cout << "Warning in Bs2JpsiPhi_Signal_v5_Robs::SetPhysicsParameters: derived parameter Apara_sq <0  and so set to zero" <<  endl;
		Apara_sq = 0.;
	}
...
double Bs2JpsiPhi_Signal_v5_Robs::EvaluateForNumericIntegral(DataPoint * measurement)
{
...
double Bs2JpsiPhi_Signal_v5_Robs::Evaluate(DataPoint * measurement)
{
	A0A0_value = measurement->GetPseudoObservable( A0A0_Obs )->GetValue();
	APAP_value = measurement->GetPseudoObservable( APAP_Obs )->GetValue();
	ATAT_value = measurement->GetPseudoObservable( ATAT_Obs )->GetValue();
	ASAS_value = measurement->GetPseudoObservable( ASAS_Obs )->GetValue();
	ImAPAT_value = measurement->GetPseudoObservable( ImAPAT_Obs )->GetValue();
	ReA0AP_value = measurement->GetPseudoObservable( ReA0AP_Obs )->GetValue();
	ImA0AT_value = measurement->GetPseudoObservable( ImA0AT_Obs )->GetValue();
	ReASAP_value = measurement->GetPseudoObservable( ReASAP_Obs )->GetValue();
	ImASAT_value = measurement->GetPseudoObservable( ImASAT_Obs )->GetValue();
	ReASA0_value = measurement->GetPseudoObservable( ReASA0_Obs )->GetValue();
	...
	//Cache amplitudes and angles terms used in cross section
	...
	if( DEBUGFLAG && (c1 || c2 || c3)  ) {
		this->DebugPrint( " Bs2JpsiPhi_Signal_v5_Robs::Evaluate() returns <=0 or nan :" , returnValue );
...
double Bs2JpsiPhi_Signal_v5_Robs::EvaluateTimeOnly(DataPoint * measurement)
{
	// Get observables into member variables
	t = measurement->GetObservable( timeName )->GetValue() - timeOffset;
	tag = (int)measurement->GetObservable( tagName )->GetValue();
	...
	bool c3 = (resolutionScale<=0.) && (t>0.) && (returnValue <= 0.) ;
	if( DEBUGFLAG && (c1 || c2 || c3)  ) {
		this->DebugPrint( " Bs2JpsiPhi_Signal_v5_Robs::EvaluateTimeOnly() returns <=0 or nan :" , returnValue );
		if( isnan(returnValue) ) throw 10;
		if( returnValue <= 0. ) throw 10;
	}
...
double Bs2JpsiPhi_Signal_v5_Robs::Normalisation(DataPoint * measurement, PhaseSpaceBoundary * boundary)
{
	if( _numericIntegralForce )
	{
		return -1.;
	}
	...
	//How it is calculated depends upon how resolution is treated
	...
	if( DEBUGFLAG && c3 )
	{
		this->DebugPrint( " Bs2JpsiPhi_Signal_v5_Robs::Normalisation() returns <=0 or nan :" , returnValue );

		if( isnan(returnValue) || returnValue <= 0. )	{	throw 10;	}
	}
...
//.......................................................
// Pre calculate the time integrals : this is because these functions are called many times for each event due to the 10 angular terms
void Bs2JpsiPhi_Signal_v5_Robs::preCalculateTimeFactors() const
{
...
//.......................................................
// Pre calculate the time integrals : this is because these functions are called many times for each event due to the 10 angular terms
void Bs2JpsiPhi_Signal_v5_Robs::preCalculateTimeIntegrals() const
{
...
vector<string> Bs2JpsiPhi_Signal_v5_Robs::PDFComponents()
{
...
double Bs2JpsiPhi_Signal_v5_Robs::EvaluateComponent( DataPoint* input, ComponentRef* Component )
{
...
//
double Bs2JpsiPhi_Signal_v5_Robs::diffXsec()  const
{
	...
	switch( componentIndex )
	{
		...
		default:	//	Everything
			...
			if( DEBUGFLAG && (xsec < 0) ) this->DebugPrintXsec( " Bs2JpsiPhi_Signal_v5_Robs_v1::diffXsec() : return value < 0 = ", xsec );
...
double Bs2JpsiPhi_Signal_v5_Robs::diffXsecTimeOnly() const
{
	preCalculateTimeFactors();
	...
	if( DEBUGFLAG && (xsec < 0) ) this->DebugPrintXsec( " Bs2JpsiPhi_Signal_v5_Robs_v1::diffXsecTimeOnly() : return value < 0 = ", xsec );
...
double Bs2JpsiPhi_Signal_v5_Robs::diffXsecNorm1() const
{
	...
	if( DEBUGFLAG && (norm < 0) ) this->DebugPrintNorm( " Bs2JpsiPhi_Signal_v5_Robs_v1::diffXsecNorm1() : return value < 0 = ", norm );
...
double Bs2JpsiPhi_Signal_v5_Robs::diffXsecCompositeNorm1( int resolutionIndex )
{
...
void Bs2JpsiPhi_Signal_v5_Robs::CacheAmplitudesAndAngles()
{
	...
	CachedA8 = AS()*AP() * ReASAP_value;
	CachedA9 = AS()*AT() * ImASAT_value;
	CachedA10= AS()*A0() * ReASA0_value;

}

//.......................................................
// New speed up method to Cache time integrals
void Bs2JpsiPhi_Signal_v5_Robs::CacheTimeIntegrals()
{
		...
		for( unsigned int ires=0; ires < 4; ++ires )
		{
			...
			switch( ires )
			{
				...
				case 2:
					resolution = resolution2 * resolutionScale;
					break;
				case 3: resolution = resolution3 * resolutionScale;
					break;
			}
...
//.......................................................
// New speed up method to Cache time integrals
void Bs2JpsiPhi_Signal_v5_Robs::deCacheTimeIntegrals( unsigned int ires, unsigned int islice )
{
...
void Bs2JpsiPhi_Signal_v5_Robs::DebugPrint( string message, double value )  const
{
	cout << "*************DEBUG OUTPUT FROM Bs2JpsiPhi_Signal_v5_Robs::DebugPrint ***************************" << endl;
...
void Bs2JpsiPhi_Signal_v5_Robs::DebugPrintXsec( string message, double value )  const
{
	cout << "*************DEBUG OUTPUT FROM Bs2JpsiPhi_Signal_v5_Robs::DebugPrintXsec ***************************" << endl;
	cout << message << value << endl <<endl;
...
void Bs2JpsiPhi_Signal_v5_Robs::DebugPrintNorm( string message, double value )  const
{
	cout << "*************DEBUG OUTPUT FROM Bs2JpsiPhi_Signal_v5_Robs::DebugPrintNorm ***************************" << endl;
	cout << message << value << endl <<endl;
