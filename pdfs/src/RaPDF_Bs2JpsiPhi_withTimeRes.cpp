// $Id: RaPDF_Bs2JpsiPhi_withTimeRes.cpp,v 1.1 2009/11/10 10:35:49 gcowan Exp $
/** @class RaPDF_Bs2JpsiPhi_withTimeRes RaPDF_Bs2JpsiPhi_withTimeRes.cpp
 *
...
//Constructor
RaPDF_Bs2JpsiPhi_withTimeRes::RaPDF_Bs2JpsiPhi_withTimeRes() : 
...
void RaPDF_Bs2JpsiPhi_withTimeRes::MakePrototypes()
{
...
bool RaPDF_Bs2JpsiPhi_withTimeRes::SetPhysicsParameters( ParameterSet * NewParameterSet )
{
	normalisationCacheValid = false;
	evaluationCacheValid = false;
...
vector<string> RaPDF_Bs2JpsiPhi_withTimeRes::GetDoNotIntegrateList()
{
...
double RaPDF_Bs2JpsiPhi_withTimeRes::Evaluate(DataPoint * measurement)
{
...
double RaPDF_Bs2JpsiPhi_withTimeRes::Normalisation(DataPoint * measurement, PhaseSpaceBoundary * boundary)
{
	// Now need to know the tag and the mistag
    	int q = (int)measurement->GetObservable( tagName )->GetValue();
    	double omega = measurement->GetObservable( mistagName )->GetValue();	
...
void RaPDF_Bs2JpsiPhi_withTimeRes::getAngularFunctions( double & f1
					, double & f2
					, double & f3
					, double & f4
					, double & f5
...
void RaPDF_Bs2JpsiPhi_withTimeRes::getTimeDependentAmplitudes(  double & AzeroAzero
						, double & AparaApara
						, double & AperpAperp
						, double & ImAparaAperp
						, double & ReAzeroApara
...
void RaPDF_Bs2JpsiPhi_withTimeRes::getTimeAmplitudeIntegrals( double & AzeroAzeroInt
...
inline double RaPDF_Bs2JpsiPhi_withTimeRes::getAzeroAzeroInt( double AzeroAzero
...
inline double RaPDF_Bs2JpsiPhi_withTimeRes::getAparaAparaInt( double AparaApara
                                                            , double expCosh_integral
                                                            , double expSinh_integral
                                                            , double expSin_integral
                                                            , double phis
...
inline double RaPDF_Bs2JpsiPhi_withTimeRes::getAperpAperpInt( double AperpAperp
                                                            , double expCosh_integral
                                                            , double expSinh_integral
                                                            , double expSin_integral
                                                            , double phis
...
inline double RaPDF_Bs2JpsiPhi_withTimeRes::getAparaAperpInt( double AparaAperp
                                                            , double expSinh_integral
...
inline double RaPDF_Bs2JpsiPhi_withTimeRes::getAzeroAparaInt( double AzeroApara
                                                            , double expCosh_integral
                                                            , double expSinh_integral
                                                            , double expSin_integral
                                                            , double phis
...
inline double RaPDF_Bs2JpsiPhi_withTimeRes::getAzeroAperpInt( double AzeroAperp
                                                            , double expSinh_integral
                                                            , double expCos_integral
                                                            , double expSin_integral
                                                            , double phis
...
inline void RaPDF_Bs2JpsiPhi_withTimeRes::getExpTrigIntegrals(
	  double & expCosh_integral
	, double & expSinh_integral
	, double & expCos_integral
	, double & expSin_integral
...
inline void RaPDF_Bs2JpsiPhi_withTimeRes::getErfPart(   double t_low
						      , double t_high
						      , double mean
						      , double sigma
						      , double tau_L
...
void RaPDF_Bs2JpsiPhi_withTimeRes::getPhysicsParameters( double & gamma
					, double & deltaGamma
					, double & deltaM
					, double & Phi_s
					, double & Azero_sq
...
double RaPDF_Bs2JpsiPhi_withTimeRes::ExpCosh( double GammaL, double GammaH
...
double RaPDF_Bs2JpsiPhi_withTimeRes::ExpSinh( double GammaL, double GammaH
	      , double mean_time_res1, double sigma_time_res1
              , double mean_time_res2, double sigma_time_res2
              , double frac_time_res1, double time) 
{
...
double RaPDF_Bs2JpsiPhi_withTimeRes::erfc( double gammaL, double gammaH
...
double RaPDF_Bs2JpsiPhi_withTimeRes::ExpCos( double gamma, double deltaM
              , double mean_time_res1, double sigma_time_res1
              , double mean_time_res2, double sigma_time_res2
              , double frac_time_res1, double time)
{
...
double RaPDF_Bs2JpsiPhi_withTimeRes::ExpSin( double gamma, double deltaM
              , double mean_time_res1, double sigma_time_res1
              , double mean_time_res2, double sigma_time_res2
              , double frac_time_res1, double time)
{
...
double RaPDF_Bs2JpsiPhi_withTimeRes::cerfRe( double gamma, double deltaM
                                           , double mean, double sigma
                                           , double time )
{
...
double RaPDF_Bs2JpsiPhi_withTimeRes::cerfIm( double gamma, double deltaM
                                           , double mean, double sigma
                                           , double time)
{
        double c  = gamma*sigma/sqrt(2.);
...
RooComplex RaPDF_Bs2JpsiPhi_withTimeRes::evalCerfApprox( double swt, double u, double c )
{
...
double RaPDF_Bs2JpsiPhi_withTimeRes::evalCerfRe( double swt, double u, double c)
{
    RooComplex z(swt * c, u + c);
    return (z.im() > -4.0) ? RooMath::FastComplexErrFuncRe(z) * exp( -u * u ) : evalCerfApprox( swt, u, c).re();
}

double RaPDF_Bs2JpsiPhi_withTimeRes::evalCerfIm( double swt, double u, double c)  {
    RooComplex z( swt * c, u + c);
    return (z.im() > -4.0) ? RooMath::FastComplexErrFuncIm(z) * exp( -u * u ) : evalCerfApprox( swt, u, c).im();
}

RooComplex RaPDF_Bs2JpsiPhi_withTimeRes::evalCerf( double swt, double u, double c ) {
    RooComplex z(swt * c, u + c);
