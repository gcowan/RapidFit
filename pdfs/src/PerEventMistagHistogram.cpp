...
#include "PerEventMistagHistogram.h"
...
PDF_CREATOR( PerEventMistagHistogram );

//Constructor
PerEventMistagHistogram::PerEventMistagHistogram( PDFConfigurator* config ) :
	...
	, total_num_entries()
{
	...
	cout << "Constructing PDF: PerEventMistagHistogram  " << endl ;
	...
	if( fileName == "" )
	{
		cout << "   No mistag histogram found: exiting " << endl ;
		exit(-1);
	}
	else
	{
		cout << "   Per-event mistag histogram requested: " << fileName << endl ;
...
*/
//Destructor
PerEventMistagHistogram::~PerEventMistagHistogram()
{
}

//Make the data point and parameter set
void PerEventMistagHistogram::MakePrototypes()
{
...
vector<string> PerEventMistagHistogram::GetDoNotIntegrateList()
{
        vector<string> list;
        return list;
}
...
bool PerEventMistagHistogram::SetPhysicsParameters( ParameterSet * NewParameterSet )
{
...
double PerEventMistagHistogram::Evaluate(DataPoint * measurement)
{
	// Observable
	mistag = measurement->GetObservable( mistagName )->GetValue();
	return this->timeMassFactor();
}


// Normlisation
double PerEventMistagHistogram::Normalisation(PhaseSpaceBoundary * boundary)
{
	(void)boundary;
	return buildPDFdenominator();
}

double PerEventMistagHistogram::buildPDFdenominator()
{
	return 1.;
}
...
double PerEventMistagHistogram::timeMassFactor( )
{
