...
Aminus(1,0)
{
  ...
  switch (LB)
  {
    ...
    case 3: barrierB=new DPBarrierL3(RB);
            break;
    default: std::cout<<"WARNING DPJpsiKSpinOne (LB): Do not know which barrier factor to use.  Using L=0 and you should check what are you doing.\n";
  ...
  switch (LR)
  {
    ...
    case 3: barrierR=new DPBarrierL3(RR);
            break;
    default: std::cout<<"WARNING DPJpsiKSpinOne (LR): Do not know which barrier factor to use.  Using L=0 and you should check what are you doing.\n";
                                   ...
                                   int twoLambdaPsi)
{
  ...
  angular+=Aminus*wigner.function(cosTheta1,-1,twoLambda/2)*
           wigner.function(cosTheta2,-1,0)*TComplex::Exp(+1.0*TComplex::I()*phi);
  // A0 has flat phi dependence, to save CPU do not multiply by complicated  1
  angular+=A0*wigner.function(cosTheta1,0,twoLambda/2)*wigner.function(cosTheta2,0,0);
  angular+=Aplus*wigner.function(cosTheta1,1,twoLambda/2)*wigner.function(cosTheta2,1,0)*
...
void DPJpsiKSpinOne::setHelicityAmplitudes(double magA0, double magAplus,
