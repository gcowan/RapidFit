...
vector<TString> StringOperations::postpend( vector<TString> input, TString suffix )
{
...
//	Is a TString empty ?
bool StringOperations::is_empty( TString input )
{
	string temp("");
...
int StringOperations::comp_TString( string test, TString input )
{
	return test.compare(input.Data());
}

//	Compare a TString to a string
int StringOperations::TString_comp( TString test, string input )
{
	return string(test.Data()).compare(input);
}

//  Useful for appendding a filename
void StringOperations::filename_append( TString input, TString *output, TString ext )
{
...
//	Wrapper
TString StringOperations::filename_append( TString input, TString ext )
{
	TString new_string;
...
string StringOperations::filename_append( string input, string ext )
{
	return string( filename_append( TString( input ), TString( ext ) ).Data() );
}

//	Originally Written by Conor Fitzpatrick
TString StringOperations::prettyPrint(Double_t value)
{
	char pretty[20];
...
//	Originally Written by Conor Fitzpatrick
TString StringOperations::prettyPrint(Double_t val, Double_t err)
{
	TString outstr = "$";
	char errstr[20];
	char valstr[20];
...
vector<TString> StringOperations::filter_names( vector<TString> all_names, string substring )
{
...
//      Pass this an array of strings and it will find all strings matching a substring
vector<TString> StringOperations::filter_names( vector<TString> all_names, TString substring )
{
...
void StringOperations::strip_strings( vector<string>* list, string ext )
{
	vector<string> new_list;
	(void) list; (void) ext; (void) new_list;
}

vector<string> StringOperations::strip_all_strings( vector<string>* list, string ext )
{
	vector<string> new_list;
...
TString StringOperations::remove_suffix( TString whole_string, TString deliminator )
{
	//	To be done
...
string StringOperations::remove_suffix( string whole_string, string deliminator )
{
	//	To be done
	(void) whole_string; (void) deliminator;
	return string();
...
vector<string> StringOperations::SplitString( string Input, char SplitCharacter )
{
...
int StringOperations::CharacterPosition( string Input, char SearchCharacter )
{
...
vector<int> StringOperations::StringPositions( string Input, string SearchString )
{
...
void StringOperations::RemoveCharacter( string & Input, char SearchCharacter )
{
...
string StringOperations::ReplaceString( string & Input, string FindString, string ReplaceString )
{
	string output;
...
void StringOperations::RemoveWhiteSpace( vector<string> & newContent )
{
...
vector<string> StringOperations::CombineUniques( vector<string> VectorOne, vector<string> VectorTwo )
{
...
int StringOperations::VectorContains( vector<TString> const& InputVector, TString const& SearchString )
{
...
TString StringOperations::CondenseStrings( vector<string> input_vec, int lolim=-1, int hilim=-1 )
{
...
vector<TString> StringOperations::GetStringContaining( vector<TString> list, TString search_str )
{
...
vector<TString> StringOperations::StripStrings( vector<TString> list, TString ext )
{
	vector<TString> output_list;
...
vector<string> StringOperations::TString2string( vector<TString> input )
{
	vector<string> output;
...
vector<TString> StringOperations::string2TString( vector<string> input )
{
	vector<TString> output;
...
TString StringOperations::RemoveSuffix( TString input, TString suffix )
{
	size_t found;
...
TString StringOperations::getFileName( TString input )
{
	string input_str( input.Data() );
...
TString StringOperations::Clean( TString input )
{
	string temp(input.Data());
