...
{
	vector<vector<Double_t> > Returnable_Array;
	vector<Double_t> temp_array = Get_Data( input_tree, Cut_String, Data_String );
	for(vector<Double_t>::iterator array_i = temp_array.begin(); array_i != temp_array.end(); ++array_i )
	{
		vector<Double_t> temp_vec;
		temp_vec.push_back( *array_i );
		Returnable_Array.push_back( temp_vec );
	}
...
{
	vector<pair<Double_t,Double_t> > temp_array = Get_Data2D( input_tree, Cut_String, Data_String );

	vector<vector<Double_t> > Returnable_Array;

	for( vector<pair<Double_t,Double_t> >::iterator array_i=temp_array.begin(); array_i!=temp_array.end(); ++array_i )
	{
		vector<Double_t> temp_vec;
		temp_vec.push_back( array_i->first );
		temp_vec.push_back( array_i->second );
		Returnable_Array.push_back( temp_vec );
	}
	return Returnable_Array;
}
