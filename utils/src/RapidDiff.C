...
int main( int argc, char* argv[] )
{
	...
	cout << setw( 23 ) << "sqrt( Diff^2+DiffErr^2 )" << endl;

	cout << endl;
	for( unsigned int i=0; i< common.size(); ++i )
	{
		cout << setw( 20 ) << common[i] << " : " << setw( 13 ) << CV_1[i] << " ± " << setw( 13 ) << Err_1[i] << "\t" << setw( 13 ) << CV_2[i] << " ± " << setw( 13 ) << Err_2[i] << "   ";
		cout << setw( 15 ) << CV_diffs[i] << "   " <<  setw( 15 ) << Err_1[i] / Err_2[i] << "   " << setw( 23 ) << sqrt( fabs(Err_1[i]*Err_1[i] - Err_2[i]*Err_2[i]) );
		cout << "   " << setw( 23 ) << sqrt( CV_diffs[i]*CV_diffs[i] + fabs(Err_1[i]*Err_1[i] - Err_2[i]*Err_2[i]) );
		cout << endl;
	}
	cout << endl;
...
string buildLatexTable( vector<TString> params, vector<double> CV1, vector<double> Err1, vector<double> CV2, vector<double> Err2 )
{
	...
	thisTable << "\\begin{tabular}{@{}l|c|c|c|c|c|c@{}}" << endl;
	thisTable << setw( 30 ) << "Parameter   & " << "   " << setw( 29 ) << " Result1    & " << setw( 29 ) << " Result2    & ";
	thisTable << setw( 18 ) << " Diff in CV & " << setw( 18 ) << " Error Ratio & " << setw( 20 ) << " $\\sqrt{ \\left|Err1^2-Err2^2\\right| }$";
	thisTable << setw( 23 ) << " & $\\sqrt{ CVDiff^2+ErrDiffSq }$ " << "   \\\\ \\hline" << endl;
	...
	for( unsigned int i=0; i< params.size(); ++i )
	{
		...
		thisTable << setw( 18 ) << CV1[i]-CV2[i] << " & " << setw( 18 ) << Err1[i] / Err2[i] << " & " << setw( 20 ) << sqrt( fabs(Err1[i]*Err1[i] - Err2[i]*Err2[i]) );
		thisTable << " & " << setw( 23 ) << sqrt( (CV1[i]-CV2[i])*(CV1[i]-CV2[i]) + fabs(Err1[i]*Err1[i] - Err2[i]*Err2[i]) );
		thisTable << "   \\\\" << endl;
