...
def _1DFC_Splitter( XML='XML.xml', STEPS=10, STEPS_PER_CORE=2 ):
	args = []

	temp_args = _1DLL_Splitter( XML, STEPS, STEPS_PER_CORE )

	num_repeats = int( TOYS_PER_GRID_POINT / TOYS_PER_GRID_NODE )

	for num in range( 0, num_repeats, 1 ):
		num2=0
		for i in temp_args:
			num2=num2+1
			local_args = []
			for j in i:
				local_args.append( j )
			local_args[-1]=str(int(local_args[-1])+int(num)*int(num2)+int(num2))
			local_args.append( str( TOYS_PER_GRID_NODE ) )
			args.append( local_args )
	print args
	return args
...
if is_ganga:
	...
	j.splitter=ArgSplitter( args = _1DFC_Splitter( FIT_XML, STEPS, STEPS_PER_CORE ) )
	#	submit the job
	j.submit()
...
if ( __name__ == '__main__' ) and ( not is_ganga ) :
	...
	args.Add( ROOT.TObjString( str(repeat_num)) )
	args.Add( ROOT.TObjString("--NuisenceModel"))
