...
#	Starting Seed and number of 1D LLscans to perform
SEED = 12345
number_of_scans = 100

#	All the possible output files right now
output_file_list = [ 'Global_Fit_Result.root', 'LLScanDatagamma.root', 'LLScanDatadeltaGamma.root', 'LLScanDataPhi_s.root',
	'LLScanDataAperp_sq.root', 'LLScanDataAzero_sq.root', 'LLScanDatadelta_para.root', 'LLScanDatadelta_perp.root', 'LLScanDatadeltaM.root' ]
...
def TOY_Splitter( XML='XML.xml', SEED='12345', number_o_toys=20):
	args = []
	real_SEED = SEED
	for i in range(0,number_o_toys,1):
		temp = []
		temp.append( str( XML ) )
...
if is_ganga:
	...
	j.splitter=ArgSplitter( args = TOY_Splitter( FIT_XML, SEED, number_of_scans ) )
	#	submit the job
	j.submit()
